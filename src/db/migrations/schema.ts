import { pgTable, foreignKey, text, serial, timestamp, unique, boolean, jsonb, doublePrecision, integer, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const auditAction = pgEnum("audit_action", ['create', 'update', 'delete', 'archive', 'restore', 'login', 'logout', 'invite', 'accept_invite', 'reject_invite', 'approve_request', 'reject_request'])
export const invitationStatus = pgEnum("invitation_status", ['pending', 'accepted', 'rejected', 'expired', 'confirmed'])
export const issueStatus = pgEnum("issue_status", ['open', 'in_progress', 'resolved', 'closed'])
export const priorityScores = pgEnum("priority_scores", ['low', 'medium', 'high', 'critical'])
export const requestStatus = pgEnum("request_status", ['pending', 'approved', 'rejected'])
export const systemEntity = pgEnum("system_entity", ['userManagement', 'trashbinManagement', 'settingsManagement', 'dashboardManagement', 'boardManagement', 'issueManagement', 'activityManagement', 'historyManagement', 'accessRequestsManagement', 'invitationsManagement', 'collectionsManagement', 'mapManagement', 'greenHeartsManagement', 'authentication', 'authorization'])


export const userCollectionAssignments = pgTable("user_collection_assignments", {
...
export const userSettings = pgTable("user_settings", {
	id: text().primaryKey().notNull(),
	userId: text("user_id").notNull(),
	settings: jsonb().default({"appearance":{"font":"Manrope","theme":"dark","liveUpdatesOnMap":true}}).notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).notNull(),
	no: serial().notNull(),
}, (table) => [
...
export const organization = pgTable("organization", {
	...
	unique("organization_email_unique").on(table.email),
]);
...
export const trashbin = pgTable("trashbin", {
	id: text().primaryKey().notNull(),
	orgId: text("org_id").notNull(),
	name: text().notNull(),
...
export const trashbinCollections = pgTable("trashbin_collections", {
	id: text().primaryKey().notNull(),
	orgId: text("org_id").notNull(),
	trashbinId: text("trashbin_id").notNull(),
...
export const trashbinStatus = pgTable("trashbin_status", {
	id: text().primaryKey().notNull(),
	trashbinId: text("trashbin_id").notNull(),
...
export const organizationSettings = pgTable("organization_settings", {
	...
	settings: jsonb().default({"backup":{"autoBackup":true,"backupFrequency":"weekly"},"general":{"location":{"lat":40,"lng":-100},"wasteLevelThreshold":"80"}}),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).notNull(),
	no: serial().notNull(),
	secret: text(),
...
export const userGreenHearts = pgTable("user_green_hearts", {
	id: text().primaryKey().notNull(),
	userId: text("user_id").notNull(),
